...
# Train the model
model = MLPClassifier(hidden_layer_sizes=(64),max_iter=500,random_state=1)
...
output.to_csv('neural.csv', index=False)

# try different architecture
# Train the model
model = MLPClassifier(hidden_layer_sizes=(32,32),max_iter=500,random_state=1)
model.fit(X_train, y_train)
...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('neural2.csv', index=False)

# try different architecture
# Train the model
model = MLPClassifier(hidden_layer_sizes=(16,16,16),max_iter=500,random_state=1)
model.fit(X_train, y_train)
...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('neural3.csv', index=False)
