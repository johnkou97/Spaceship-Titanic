...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('random_forest_1000.csv', index=False)

# Train with 2000 trees
...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('random_forest_2000.csv', index=False)

# Train with 1000 trees and entropy
...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('random_forest_1000_entropy.csv', index=False)

# Train with 3000 trees 
model = RandomForestClassifier(n_estimators=3000, random_state=1)
model.fit(X_train, y_train)
...
output = pd.DataFrame({'PassengerId': test_data.PassengerId,
                            'Transported': test_preds})
output.to_csv('random_forest_3000.csv', index=False)
